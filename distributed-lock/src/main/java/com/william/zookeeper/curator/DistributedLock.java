...
public class DistributedLock {
     ...
     * 获取分布式锁
     */
    public static void acquireDistributedLock(String path) {
     ...
     * 释放分布式锁
     */
    public static boolean releaseDistributedLock(String path) {
        try {
            String keyPath = "/" + ROOT_PATH_LOCK + "/" + path;
            CuratorFramework curatorFramework = build();
            curatorFramework.start();
     ...
     * 创建 watcher 事件
     */
    private static void addWatcher(String path) throws Exception {
        String keyPath;
